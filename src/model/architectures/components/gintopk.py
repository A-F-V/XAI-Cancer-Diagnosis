...
#    return GINConv(nn=Seq(BatchNorm(in_channels), ReLU(), Lin(in_channels, in_channels), BatchNorm(in_channels), #ReLU(), Lin(in_channels, out_channels)))

def _create_convolution(in_channels, out_channels):
    return GCNConv(in_channels, out_channels, improved=True)


def _create_activation(channels):
    return Seq(BatchNorm1d(channels), ReLU())
...
class GCNTopK(torch.nn.Module):
    def __init__(self, input_width, hidden_width, output_width, conv_depth=4):
        ...
        self.act = ModuleList([_create_activation(self.hw) for i in range(self.conv_depth)])
