...
def _create_convolution(in_channels, out_channels):
    return GINConv(nn=Seq(Lin(in_channels, out_channels), ReLU(), Lin(out_channels, out_channels), ReLU()))
...
class GINTopK(torch.nn.Module):
    def __init__(self, input_width, hidden_width, output_width, conv_depth=4):
        ...
        self.predictor = Sequential(Linear(2*hidden_width, hidden_width),
                                    ReLU(),
                                    Dropout(),
                                    Linear(hidden_width, hidden_width//2),
                                    ReLU(),
                                    Linear(hidden_width//2, output_width))
    ...
    def forward(self, x, edge_index, batch):
        ...
        for i in range(self.conv_depth):
            ...
            x, edge_index, _, batch = self.pool[i](x, edge_index, None, batch)
