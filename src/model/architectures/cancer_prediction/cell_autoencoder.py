...
class Conv(nn.Module):
    def __init__(self, in_c, out_c, depth=2):
        ...
        self.convs = nn.ModuleList([nn.Conv2d(in_channels=in_c if i == 0 else out_c, out_channels=out_c,
                                   kernel_size=3, padding=1, stride=2 if i == depth-1 else 1) for i in range(depth)])
        self.relus = nn.ModuleList([nn.ReLU(inplace=True) for i in range(depth)])
        self.bns = nn.ModuleList([nn.BatchNorm2d(out_c) for i in range(depth)])
        self.depth = depth
