...
class GCNRefx(torch.nn.Module):
    def __init__(self, input_width, hidden_width, output_width, conv_depth=4, input_dropout=0.1):
        ...
        self.transform = ModuleList([_create_transform((self.iw if i == 0 else self.hw), self.hw, dropout=input_dropout)  # input_dropout if i == 0 else 0)
                                    for i in range(self.conv_depth)])
